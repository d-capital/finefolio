...
def fine_folio_core(stocks_to_work, user_capital, start, end):
    ...
    elif stocks_to_work == 'short_list':
        asset_list = short_list
    else:
    ...
    for i in asset_list:
        ...
        asset = asset[asset.BOARDID.isin(['RPMO', 'RPMA'])]
    ...
    for stock in asset_list:
        ...
        #last_close*lot*x = optimal_weight[i]*user_capital
        x = (optimal_weights[stock]*user_capital)/(last_close[0]*int(lots.loc[stock][0]))
        lot_to_use = int(lots.loc[stock][0])
    ...
    optimal_weights_2 = optimal_weights.to_frame()
    used_lots = used_lots.to_frame()
    used_quantities = used_quantities.to_frame()
    frames = [optimal_weights_2, used_lots, used_quantities]
    ...
    # plt.ylabel('Return')
    # plt.show()
    return optimal_weights, optimal_weights_2

def backtest(stocks_to_work, optimal_weights, user_capital, start_b, end_b):
    ...
    #start_b = dt.datetime(2019, 1, 1)
    #end_b = dt.datetime(2019, 12, 31)
    ...
    year_dates = pd.date_range('2019-06-19', '2020-06-19', freq='D')
    ...
    for i, allo in zip(asset_list, optimal_weights):
        ...
        asset_b = asset_b[asset_b.BOARDID.isin(['RPMO','RPMA'])]
