...
def send_text(message):
    ...
    if send_text_0(message) > 0:
        ...
        send_text_2(message)
    send_text_1(message, user_capital_1)
...
def send_text_1(message, user_capital, moex_list='moex_list', blue='blue_chips_moex'):
    if message.text.lower() == 'голубые фишки':
        bot.send_message(message.chat.id,
                         'Я начал просчитывать оптимальный портфель, это займет 5-10 минут. Можете пока выпить чай или кофе. В экселе мой создатель делал это часами. :-)')
