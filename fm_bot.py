...
def send_text_1(message, user_capital, moex_list='moex_list', blue='blue_chips_moex'):
    if message.text.lower() == 'голубые фишки':
        ...
        opt_distrib = fine_folio_2.fine_folio_core(blue, user_capital, start=dt.datetime(2019, 6, 19), end=dt.datetime(2020, 6, 19))
        bot.send_message(message.chat.id, 'Ваш оптимальный портфель на 2020 год (не торговая рекомендация): \n{}'.format(opt_distrib[1]))
        ...
        bot.send_message(message.chat.id, 'Если бы вы вложили тот объем денег год назад, '
                                          'сейчас у вас бы было {} р. Не забывайте - это слишком оптимистичная цифра, целевая доходность системы 20%, а торговля на финансовых рынках - всегда риск.'.format(new_capital))
        ...
        opt_distrib = fine_folio_2.fine_folio_core(moex_list, user_capital, start=dt.datetime(2019, 6, 19), end=dt.datetime(2020, 6, 19))
        bot.send_message(message.chat.id, 'Ваш оптимальный портфель на 2020 год (не торговая рекомендация): \n{}'.format(opt_distrib[1]))
